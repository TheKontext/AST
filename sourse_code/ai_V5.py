...
def ans(message):
    text = '''
    я отправлю тебе ряд и ответов на вопросы о человеке,на их основе отправь Только ряд профессий(2-5 штук),
    которые могут подойти этому человеку,
    с их небольшим описанием, выдели названия проффессий в <b></b>,выяви сильные, проессиональные стороны человека,
    подзаголовок Сильные профессиональные стороны выдели в <b></b>: 
    '''
    messages = [
        {"role": "user", "text": text + message}
    ]
